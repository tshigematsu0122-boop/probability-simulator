...
# ------------------------------
# matplotlib 日本語対応（画像描画用）
# ------------------------------
font_path = "NotoSansJP-Black.ttf"  # リポジトリに置いたフォントファイル
...
# 入力欄（デフォルト値を変更）
# ------------------------------
n1_str = st.text_input("確率分母", "319.7")  # デフォルト値を319.7
try:
    n1 = float(n1_str)
    if n1 <= 0:
...
n2 = st.number_input("回転数（ゲーム数）", min_value=1, value=320, step=1)  # デフォルト値320
n3 = st.number_input("当たり回数", min_value=0, value=1, step=1)  # デフォルト値1
...
practical_str = f"1/{(n/k if k>0 else 0):.2f}".rstrip('0').rstrip('.')
...
# ------------------------------
# グラフ描画（色付き改善版、濃く、ラベル太字、画像として描画）
# ------------------------------
st.subheader("正規分布近似グラフ（画像描画）")
...
# 軸ラベルを画像として描画
ax.set_xlabel("当たり回数", fontproperties=prop)
ax.set_ylabel("確率", fontproperties=prop)
ax.set_title("正規分布近似", fontproperties=prop)
